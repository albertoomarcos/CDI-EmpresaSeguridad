...
class DomoticaItemsControl1: AppCompatActivity() {
    ...
    private fun onOff() {
        ...
        val item1 = findViewById<ConstraintLayout>(R.id.item1)
        val on1 = findViewById<ImageView>(R.id.on1)
        item1.setBackgroundColor(colorActivate)
        on1.setImageDrawable(imageActivate)
        on1.setOnClickListener {
            if((it as ImageView).drawable == imageActivate) {
                item1.setBackgroundColor(colorDeactivate)
        ...
        val on2 = findViewById<ImageView>(R.id.on2)
        on2.setOnClickListener {
            if((it as ImageView).drawable == imageActivate) {
                item2.setBackgroundColor(colorDeactivate)
                it.setImageDrawable(imageDeactivate)
            } else {
                item2.setBackgroundColor(colorActivate)
                it.setImageDrawable(imageActivate)
            }
        }
        ...
        val on4 = findViewById<ImageView>(R.id.on4)
        on4.setOnClickListener {
            if((it as ImageView).drawable == imageActivate) {
                item4.setBackgroundColor(colorDeactivate)
                it.setImageDrawable(imageDeactivate)
            } else {
                item4.setBackgroundColor(colorActivate)
                it.setImageDrawable(imageActivate)
            }
        }
